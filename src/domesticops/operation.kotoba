(ns domesticops.operation
  "OperationActor -- one domestic-employment operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (DomesticOps-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Domestic Employment
  ...
  One graph run = one domestic-employment operation (intake -> advise
  ...
  decision to a human operator (household employer/agency coordinator).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:assignment/dispatch`/`:assignment/pay` ALWAYS reach
  this node when the governor is clean -- see `domesticops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [domesticops.domesticopsllm :as domesticopsllm]
            [domesticops.governor :as governor]
...
(defn build
  ...
  `domesticops.store/Store`).
  opts:
    :advisor      -- a `domesticops.domesticopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (domesticopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; DomesticOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (domesticopsllm/-advise advisor store request)]
            {:proposal p :audit [(domesticopsllm/trace request p)]})))

      ;; Domestic Employment Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
